...
L = 4.8950630 + 0.017202791698 * n; % mean longitude
g = 6.2400600 + 0.0172019699 * n; % mean anomaly
l = L + 0.03341607 * sin(g) + 0.00034894 * sin(2*g) - 0.0001134 - 0.0000203 * sin(Omega); % ecliptic longitude
...
gmst = 6.6974243242 + 0.0657098283 * n + mod(hour_x'-2,24); % GMT
lmst = (gmst*15 + longitude)*pi/180; % local solar time
hour_angle = lmst-ra;
theta_z = acos(cosd(latitude)*cos(hour_angle).*cos(delta)+sin(delta)*sind(latitude)); % zenith
...
azimuth = atan(-sin(hour_angle)./(tan(delta).*cosd(latitude) - sind(latitude).*cos(hour_angle))); % azimuth angle of sun
incident_angle=real(acosd(sind(zenith_angle).*sind(panel_pitch).*cosd(panel_azimuth-azimuth)+cosd(zenith_angle).*cosd(panel_pitch)))'; % solar incident angle taking into account panel tilt and azimuth
...
% multiplier for clear-sky irradiance. The Hammer algorithm has been used 
...
% irradiances with DISORT externally and would take several hours to run.

% top of atmosphere normal irradiance
eccentricity = 1 + 0.03344*cos(2*pi*day/365.25 - 0.048869);
solar_constant = 1367*eccentricity; 
clear eccentricity

% Kasten airmass formula as a function of zenith angle
airmass = zeros(1,length(month_record));
airmass(zenith_angle<=90) = (1 - height_above_sea_level/10000)*((cosd(zenith_angle(zenith_angle<=90))+0.50572*(96.07995-zenith_angle(zenith_angle<=90)).^(-1.6364)).^(-1)); % Airmass as function of zenith angle, from Kasten and Young (1989) Applied Optics 28:47354738
...
% get values from TIF file of Linke turbidities% script to get values from TIF files of Linke turbidities
% Linke Turbidity = greyscale_value/20
% to keep memory as free as possible we will delete variables as soon as  they become unnecessary and only save at the lon/lat point of interest Output is a 12-element vector of LT values, one per month.lat_index=round((-latitude+1/24+90)*4320/360); %round the lattitude
lon_index = round((longitude+1/24+180)*4320/360); %round the longitude
lat_index=round((-latitude+1/24+90)*4320/360); %round the latitude
